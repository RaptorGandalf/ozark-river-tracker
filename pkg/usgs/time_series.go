...
func (t *TimeSeriesResponse) GetCoordinates() (float64, float64, error) {
	...
	if len(t.Value.TimeSeries) == 0 {
		err = errors.New("No time series returned for gauge")
	} else {
		latitude = t.Value.TimeSeries[0].SourceInfo.GeoLocation.GeogLocation.Latitude
		longitude = t.Value.TimeSeries[0].SourceInfo.GeoLocation.GeogLocation.Longitude
	}
